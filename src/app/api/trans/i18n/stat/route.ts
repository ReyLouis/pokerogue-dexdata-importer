...
const OUTPUT_DIR = path.join(process.cwd(), 'public', 'json','stat');
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const { locale, data } of locales) {
      const outputFilePath = path.join(OUTPUT_DIR, locale, 'stat.json');
      ...
      for (const [key, value] of Object.entries(abilityTranslations.Stat)) {
        transformedData.Stat[key] = value;
      }
