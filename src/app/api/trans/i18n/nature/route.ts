...
import { nature as dataDE } from '@/locales/de/nature';
import { nature as dataEN } from '@/locales/en/nature';
import { nature as dataES } from '@/locales/es/nature';
import { nature as dataFR } from '@/locales/fr/nature';
import { nature as dataIT } from '@/locales/it/nature';
import { nature as dataKO } from '@/locales/ko/nature';
import { nature as dataPT_BR } from '@/locales/pt_BR/nature';
import { nature as dataZH_CN } from '@/locales/zh_CN/nature';
import { nature as dataZH_TW } from '@/locales/zh_TW/nature';
...
const OUTPUT_DIR = path.join(process.cwd(), 'public', 'json','nature');
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const { locale, data } of locales) {
      const outputFilePath = path.join(OUTPUT_DIR, locale, 'nature.json');
      ...
      for (const [key, value] of Object.entries(dataTranslations)) {
        transformedData.Nature[key.toUpperCase()] = value;
      }
